...
def cluster_jets(dataloaders):
    ...
    max_num_constituents = 25
    number_of_unique_pairs = int(max_num_constituents * (max_num_constituents - 1) / 2)
    ...
    def _calculate_q(final_states, scattered_electron):
        sigma_h = np.array(
            [
                np.sum(event[:, 3] - event[:, 2])
                for event in final_states
                if any(np.abs(event[:, 0]) != 0)
            ]
        )
        scattered_electron_momentum = np.sqrt(
            scattered_electron["px"] ** 2
            + scattered_electron["py"] ** 2
            + scattered_electron["pz"] ** 2
        )
        scattered_electron_theta = np.arccos(
            scattered_electron["pz"] / scattered_electron_momentum
        )
        sigma_eprime = scattered_electron["E"] * (1 - np.cos(scattered_electron_theta))
        sigma_tot = sigma_h + sigma_eprime
        beam_electron_momentum = {
            "px": np.zeros(len(sigma_tot)),
            "py": np.zeros(len(sigma_tot)),
            "pz": -sigma_tot / 2.0,
            "E": sigma_tot / 2.0,
        }
    ...
    def _calculate_jet_features(jet, q, elec_px, elec_py, elec_pz):
        ...
        subset = constituents[:max_num_constituents]
        # jet.multiplicty = len(constituents)
        for part_i, constituent in enumerate(constituents):
            if part_i < max_num_constituents:
                for part_j in range(part_i + 1, len(subset)):
                    constituent_j = constituents[part_j]
                    delta_phi = constituent.phi() - constituent_j.phi()
                    delta_phi = (delta_phi + np.pi) % (2 * np.pi) - np.pi

                    R_L_ij = np.sqrt(
                        delta_phi**2 + (constituent.eta() - constituent_j.eta()) ** 2
                    )
                    energy_weight = (
                        constituent.pt() * constituent_j.pt() / (jet.pt() ** 2)
                    )

                    R_L.append(R_L_ij)
                    EEC_energyweight.append(energy_weight)
                h_jet_dot = np.abs(
                    jet.px() * constituent.px()
                    + jet.py() * constituent.py()
                    + jet.pz() * constituent.pz()
                )
                jet_magnitude = np.sqrt(
                    jet.px() * jet.px() + jet.py() * jet.py() + jet.pz() * jet.pz()
                )
                zh.append(h_jet_dot / (jet_magnitude * jet_magnitude))

                h_jet_cross_x = (
                    jet.py() * constituent.pz() - jet.pz() * constituent.py()
                )
                h_jet_cross_y = (
                    jet.pz() * constituent.px() - jet.px() * constituent.pz()
                )
                h_jet_cross_z = (
                    jet.px() * constituent.py() - jet.py() * constituent.px()
                )
                cross_magnitude = np.sqrt(
                    h_jet_cross_x**2 + h_jet_cross_y**2 + h_jet_cross_z**2
                )
                ...
                h_photon_cross_magnitude = np.sqrt(
                    h_photon_cross_x**2 + h_photon_cross_y**2 + h_photon_cross_z**2
                )
        ...
        jet.zh = np.concatenate((zh, np.zeros(max_num_constituents - len(zh))))
        jet.jt = np.concatenate((jt, np.zeros(max_num_constituents - len(jt))))
        jet.jt_photon = np.concatenate(
            (jt_photon, np.zeros(max_num_constituents - len(jt_photon)))
        )
        # Calculating zjet
        P = np.array(
        ...
        jet.EEC_energyweight = np.concatenate(
            (EEC_energyweight, np.zeros(number_of_unique_pairs - len(EEC_energyweight)))
        )
        jet.R_L = np.concatenate((R_L, np.zeros(number_of_unique_pairs - len(R_L))))
    ...
    def _take_all_jets(jets):
        ...
        if not jets:
            return (
                np.zeros((max_num_jets, 11)),
                np.zeros((max_num_jets, max_num_constituents)),
                np.zeros((max_num_jets, max_num_constituents)),
                np.zeros((max_num_jets, max_num_constituents)),
                np.zeros((max_num_jets, number_of_unique_pairs)),
                np.zeros((max_num_jets, number_of_unique_pairs)),
            )
        jet_array = []
        ...
        for i in range(max_num_jets):
            if i < len(jets):
                ...
                zh_array.append(np.zeros(max_num_constituents))
                jt_array.append(np.zeros(max_num_constituents))
                jt_photon_array.append(np.zeros(max_num_constituents))
                EEC_energyweight_array.append(np.zeros(number_of_unique_pairs))
                RL_array.append(np.zeros(number_of_unique_pairs))
            jet_array.append(jet_info)
        return (
            np.array(jet_array),
            np.array(zh_array),
            np.array(jt_array),
            np.array(jt_photon_array),
            np.array(EEC_energyweight_array),
            np.array(RL_array),
        )
    ...
    for dataloader_name, data in dataloaders.items():
        ...
        for i, event in enumerate(cartesian):
            ...
            for jet in sorted_jets:
                _calculate_jet_features(
                    jet,
                    q[i],
                    electron_momentum["px"][i],
                    electron_momentum["py"][i],
                    electron_momentum["pz"][i],
                )
    ...
    for feature in range(dataloaders["Rapgap"].part.shape[-1]):
        ...
        if flags.sys:
            nominal, binning = np.histogram(
                ...
                bins=nbins,
                density=True,
                weights=(dataloaders["Rapgap"].weight * data_weights["Rapgap"])
                .reshape(-1, 1, 1)
            ...
            nominal_closure, _ = np.histogram(
                ...
                bins=binning,
                density=True,
                weights=(dataloaders["Djangoh"].weight)
                .reshape(-1, 1, 1)
                .repeat(num_part, axis=1)
                .reshape(-1)[dataloaders["Djangoh"].mask],
            )
            ...
            for sys, sys_weights in data_weights.items():
                ...
                sys_var, _ = np.histogram(
                    ...
                    bins=binning,
                    density=True,
                    weights=(dataloaders[sample_name].weight * sys_weights)
                    .reshape(-1, 1, 1)
                    .repeat(num_part, axis=1)
                    .reshape(-1)[dataloaders[sample_name].mask],
                )
        ...
        if flags.reco:
            ...
            weights["data"] = (
                ...
                .reshape(-1, 1, 1)
                .repeat(num_part, axis=1)
                .reshape(-1)[dataloaders["data"].mask]
            )
...
def plot_qtQ(flags, dataloaders, data_weights, version):
    ...
    weights = {
        "Rapgap_unfolded": (
            dataloaders["Rapgap"].weight * dataloaders["Rapgap"].unfolded_weights
        )[dataloaders["Rapgap"].jet[:, 0] > 0],
    ...
    feed_dict = {
        "Rapgap_unfolded": dataloaders["Rapgap"].jet[:, 0][
            dataloaders["Rapgap"].jet[:, 0] > 0
        ],
        "Rapgap": dataloaders["Rapgap"].jet[:, 0][dataloaders["Rapgap"].jet[:, 0] > 0],
        "Djangoh": dataloaders["Djangoh"].jet[:, 0][
            dataloaders["Djangoh"].jet[:, 0] > 0
        ],
    }

    if flags.reco:
        feed_dict["data"] = dataloaders["data"].jet[:, 0][
            dataloaders["data"].jet[:, 0] > 0
        ]
    ...
    feed_dict = {
        "Rapgap_unfolded": dataloaders["Rapgap"].jet[:, 1][
            dataloaders["Rapgap"].jet[:, 0] > 0
        ],
        "Rapgap": dataloaders["Rapgap"].jet[:, 1][dataloaders["Rapgap"].jet[:, 0] > 0],
        "Djangoh": dataloaders["Djangoh"].jet[:, 1][
            dataloaders["Djangoh"].jet[:, 0] > 0
        ],
    }

    if flags.reco:
        feed_dict["data"] = dataloaders["data"].jet[:, 1][
            dataloaders["data"].jet[:, 0] > 0
        ]
        weights["data"] = dataloaders["data"].weight[dataloaders["data"].jet[:, 0] > 0]
    ...
    def _get_qtQ(jet, elec):
        qt = np.sqrt(
            (
                jet[:, 0] * np.cos(jet[:, 2])
                + elec[:, 2] * np.exp(elec[:, 0] / 2.0) * np.cos(elec[:, 4])
            )
            ** 2
            + (
                jet[:, 0] * np.sin(jet[:, 2])
                + elec[:, 2] * np.exp(elec[:, 0] / 2.0) * np.sin(elec[:, 4])
            )
            ** 2
        )
        return qt / np.exp(elec[:, 0] / 2.0)

    feed_dict = {
        "Rapgap_unfolded": _get_qtQ(
            dataloaders["Rapgap"].jet, dataloaders["Rapgap"].event
        )[dataloaders["Rapgap"].jet[:, 0] > 0],
        "Rapgap": _get_qtQ(dataloaders["Rapgap"].jet, dataloaders["Rapgap"].event)[
            dataloaders["Rapgap"].jet[:, 0] > 0
        ],
        "Djangoh": _get_qtQ(dataloaders["Djangoh"].jet, dataloaders["Djangoh"].event)[
            dataloaders["Djangoh"].jet[:, 0] > 0
        ],
    }

    if flags.reco:
        feed_dict["data"] = _get_qtQ(
            dataloaders["data"].jet, dataloaders["data"].event
        )[dataloaders["data"].jet[:, 0] > 0]
...
def plot_deltaphi(flags, dataloaders, data_weights, version):
    ...
    def compute_histogram(dataset_name, weights=None):
        ...
        data = get_deltaphi(
            dataloaders[dataset_name].jet, dataloaders[dataset_name].event
        )[valid_indices]
    ...
    total_unc = None
    if flags.sys:
        nominal, _ = compute_histogram(
            "Rapgap", dataloaders["Rapgap"].weight * data_weights["Rapgap"]
        )
        ...
        for sys, sys_weights in data_weights.items():
            ...
            sample_name = "Rapgap" if sys == "closure" else sys
            sys_hist, _ = compute_histogram(
                sample_name, dataloaders[sample_name].weight * sys_weights
            )
    ...
    weights = {
        data_name: (dataloaders["Rapgap"].weight * data_weights[weight_name])[
            dataloaders["Rapgap"].jet[:, 0] > 0
        ],
        "Rapgap": dataloaders["Rapgap"].weight[dataloaders["Rapgap"].jet[:, 0] > 0],
        "Djangoh": dataloaders["Djangoh"].weight[dataloaders["Djangoh"].jet[:, 0] > 0],
    ...
    feed_dict = {
        data_name: get_deltaphi(dataloaders["Rapgap"].jet, dataloaders["Rapgap"].event)[
            dataloaders["Rapgap"].jet[:, 0] > 0
        ],
        "Rapgap": get_deltaphi(dataloaders["Rapgap"].jet, dataloaders["Rapgap"].event)[
            dataloaders["Rapgap"].jet[:, 0] > 0
        ],
        "Djangoh": get_deltaphi(
            dataloaders["Djangoh"].jet, dataloaders["Djangoh"].event
        )[dataloaders["Djangoh"].jet[:, 0] > 0],
    }

    if flags.reco:
        feed_dict["data"] = get_deltaphi(
            dataloaders["data"].jet, dataloaders["data"].event
        )[dataloaders["data"].jet[:, 0] > 0]
...
def plot_jet_pt(flags, dataloaders, data_weights, version, lab_frame=True):
    ...
    if flags.sys:
        nominal, _ = compute_histogram(
            "Rapgap", dataloaders["Rapgap"].weight * data_weights["Rapgap"], lab_frame
        )
        nominal_closure, _ = compute_histogram(
            "Djangoh", dataloaders["Djangoh"].weight, lab_frame
        )
        ...
        for sys, sys_weights in data_weights.items():
            ...
            sample_name = "Rapgap" if sys == "closure" else sys
            sys_hist, _ = compute_histogram(
                sample_name, dataloaders[sample_name].weight * sys_weights, lab_frame
            )
    ...
    if lab_frame:
        ...
        feed_dict = {
            data_name: dataloaders["Rapgap"].jet[:, 0][
                dataloaders["Rapgap"].jet[:, 0] > 0
            ],
            "Rapgap": dataloaders["Rapgap"].jet[:, 0][
                dataloaders["Rapgap"].jet[:, 0] > 0
            ],
            "Djangoh": dataloaders["Djangoh"].jet[:, 0][
                dataloaders["Djangoh"].jet[:, 0] > 0
        ...
        weights = {
            data_name: (dataloaders["Rapgap"].weight * data_weights[weight_name])[
                dataloaders["Rapgap"].jet_breit[:, 0] > 0
            ],
            "Rapgap": dataloaders["Rapgap"].weight[
                dataloaders["Rapgap"].jet_breit[:, 0] > 0
            ],
            "Djangoh": dataloaders["Djangoh"].weight[
                dataloaders["Djangoh"].jet_breit[:, 0] > 0
            ],
        }

        feed_dict = {
            data_name: dataloaders["Rapgap"].jet_breit[:, 0][
                dataloaders["Rapgap"].jet_breit[:, 0] > 0
            ],
            "Rapgap": dataloaders["Rapgap"].jet_breit[:, 0][
                dataloaders["Rapgap"].jet_breit[:, 0] > 0
            ],
            "Djangoh": dataloaders["Djangoh"].jet_breit[:, 0][
                dataloaders["Djangoh"].jet_breit[:, 0] > 0
            ],
        }

        if flags.reco:
            feed_dict["data"] = dataloaders["data"].jet_breit[:, 0][
                dataloaders["data"].jet_breit[:, 0] > 0
            ]
    ...
    fig, ax = utils.HistRoutine(
        ...
        xlabel=(
            ...
            if lab_frame
            else r"Breit frame Jet $p_\mathrm{T}$ [GeV]"
        ),
        weights=weights,
        logy=True,
        logx=True,
...
def plot_tau(flags, dataloaders, data_weights, version):
    ...
    feed_dict = {
        ...
        "Djangoh": dataloaders["Djangoh"].jet[:, 4][
            dataloaders["Djangoh"].jet[:, 0] > 0
        ],
    }

    if flags.reco:
        feed_dict["data"] = dataloaders["data"].jet[:, 4][
            dataloaders["data"].jet[:, 0] > 0
        ]
...
def plot_zjet(flags, dataloaders, data_weights, version, frame="lab"):
    ...
    def compute_histogram(dataset_name, weights=None, frame="lab"):
        if frame == "breit":
            ...
            data = ak.drop_none(
                ak.mask(dataloaders[dataset_name].all_jets[:, :, 9], mask)
            )
            ...
            data = ak.drop_none(
                ak.mask(dataloaders[dataset_name].all_jets_breit[:, :, 7], mask)
            )
            num_jets_per_event = ak.count(data, axis=1)
            data = ak.flatten(data)
        if weights is not None:
    ...
    if flags.sys:
        nominal, _ = compute_histogram(
            "Rapgap", dataloaders["Rapgap"].weight * data_weights["Rapgap"], frame
        )
        nominal_closure, _ = compute_histogram(
            "Djangoh", dataloaders["Djangoh"].weight, frame
        )
        ...
        for sys, sys_weights in data_weights.items():
            ...
            sample_name = "Rapgap" if sys == "closure" else sys
            sys_hist, _ = compute_histogram(
                sample_name, dataloaders[sample_name].weight * sys_weights, frame
            )
    ...
    if frame == "breit":
        ...
        Rapgap_data = ak.drop_none(
            ak.mask(dataloaders["Rapgap"].all_jets_breit[:, :, 7], Rapgap_mask)
        )
        ...
        Djangoh_data = ak.drop_none(
            ak.mask(dataloaders["Djangoh"].all_jets_breit[:, :, 7], Djangoh_mask)
        )
        ...
        weights = {
            ...
            "Rapgap": np.repeat(
                dataloaders["Rapgap"].weight, num_Rapgap_jets_per_event, axis=0
            ),
            "Djangoh": np.repeat(
                dataloaders["Djangoh"].weight, num_Djangoh_jets_per_event, axis=0
            ),
        }
        ...
        if flags.reco:
            ...
            data = ak.drop_none(
                ak.mask(dataloaders["data"].all_jets_breit[:, :, 7], data_mask)
            )
            data = ak.flatten(data)
            feed_dict["data"] = data
    else:
        ...
        Rapgap_data = ak.drop_none(
            ak.mask(dataloaders["Rapgap"].all_jets[:, :, 9], Rapgap_mask)
        )
        num_Rapgap_jets_per_event = ak.count(Rapgap_data, axis=1)
        Rapgap_data = ak.flatten(Rapgap_data)
        ...
        Djangoh_data = ak.drop_none(
            ak.mask(dataloaders["Djangoh"].all_jets[:, :, 9], Djangoh_mask)
        )
        num_Djangoh_jets_per_event = ak.count(Djangoh_data, axis=1)
        Djangoh_data = ak.flatten(Djangoh_data)
        ...
        if flags.reco:
            ...
            data = ak.drop_none(
                ak.mask(dataloaders["data"].all_jets[:, :, 7], data_mask)
            )
            data = ak.flatten(data)
            feed_dict["data"] = data
...
def cluster_breit(flags, dataloaders):
    ...
    def boost_particles(final_states, scattered_electron):
        ...
        for event in final_states:
            nonzero_particles = np.array(
                [part for part in event if np.abs(part[0]) != 0]
            )
            particle_vectors.append(
                [
                    vector.obj(px=part[0], py=part[1], pz=part[2], energy=part[3])
                    for part in nonzero_particles
                ]
            )
        ...
        for q, event_particles in zip(q_list, particle_vectors):
            ...
            xBoost = vector.obj(
                px=q.px / q.pt, py=q.py / q.pt, pz=q.pt / Sigma, energy=q.pt / Sigma
            )
    ...
    def calculate_eec(parts, q, i, event, scattered_electron):
        ...
        x_B = (
            scattered_electron["E"]
            * np.divide(1 + np.cos(scattered_electron_theta), 2 * y * 920)
        )[i]  # also need to divide by E_proton
    ...
    for dataloader_name, data in dataloaders.items():
        ...
        if flags.eec:
            ...
            for i, event in enumerate(boosted_vectors):
                ...
                eec, E_wgt, theta = calculate_eec(
                    event, q[i], i, data.event, electron_momentum
                )
            ...
            # Store the jet features in the dataloader
            data.eec = np.array(
                list_of_eec, dtype=np.float32
            )  # (n_event, n_max_parts, 1)
            # print(f"----------------- Done working with {dataloader_name} -------------------")
        ...
        def _take_all_jets(jets, maxjets=5):
            ...
            jet[:, :, 0] = -np.array(
                list(itertools.zip_longest(*jets.pt.to_list(), fillvalue=0))
            ).T[:, :maxjets]
            jet[:, :, 1] = np.array(
                list(itertools.zip_longest(*jets.eta.to_list(), fillvalue=0))
            ).T[:, :maxjets]
            jet[:, :, 2] = np.array(
                list(itertools.zip_longest(*jets.phi.to_list(), fillvalue=0))
            ).T[:, :maxjets]
            jet[:, :, 3] = np.array(
                list(itertools.zip_longest(*jets.E.to_list(), fillvalue=0))
            ).T[:, :maxjets]
            jet[:, :, 4] = np.array(
                list(itertools.zip_longest(*jets.px.to_list(), fillvalue=0))
            ).T[:, :maxjets]
            jet[:, :, 5] = np.array(
                list(itertools.zip_longest(*jets.py.to_list(), fillvalue=0))
            ).T[:, :maxjets]
            jet[:, :, 6] = np.array(
                list(itertools.zip_longest(*jets.pz.to_list(), fillvalue=0))
            ).T[:, :maxjets]
            return jet
        ...
        dataloaders[dataloader_name].all_jets_breit = calculate_zjet(
            dataloaders[dataloader_name].all_jets_breit,
            dataloaders[dataloader_name].event,
        )


def plot_EEC_RL(flags, dataloaders, version):
    ...
    # Set binning
    binning = np.logspace(-2, 0, 15)
    scaled_binning = np.logspace(-1, 1.5, 15)
    # Compute nominal and closure histograms if systematic uncertainties are enabled
    # total_unc = None
    # if flags.sys:
    ...
    # Prepare weights and data for plotting
    pt_ranges = [(0, None), (10, 30), (30, 50), (50, 70), (70, 100)]
    y_limits = [(0, 2), (0, 2), (0, 3.5), (0, 6.5), (0, 8)]

    for i, pt_range in enumerate(pt_ranges):
        weights = {}
        feed_dict = {}

        pt_low, pt_high = pt_range[0], pt_range[1]
        # Configuring Rapgap
        if pt_high == None:
            Rapgap_pt_mask = dataloaders["Rapgap"]["jet_pt"] > pt_low
        else:
            Rapgap_pt_mask = (dataloaders["Rapgap"]["jet_pt"] > pt_low) & (
                dataloaders["Rapgap"]["jet_pt"] < pt_high
            )

        Rapgap_qt_mask_nonrepeated = (
            dataloaders["Rapgap"]["jet_qt"][Rapgap_pt_mask]
            / dataloaders["Rapgap"]["jet_pt"][Rapgap_pt_mask]
            < 0.3
        )
        Rapgap_mean_pt = np.mean(
            dataloaders["Rapgap"]["jet_pt"][Rapgap_pt_mask][Rapgap_qt_mask_nonrepeated]
        )
        Rapgap_qt_mask = np.repeat(
            (
                ak.mask(dataloaders["Rapgap"]["jet_qt"], Rapgap_pt_mask)
                / ak.mask(dataloaders["Rapgap"]["jet_pt"], Rapgap_pt_mask)
                < 0.3
            )[:, :, np.newaxis],
            dataloaders["Rapgap"]["R_L"].shape[2],
            axis=2,
        )
        Rapgap_valid_jet_mask = (
            ak.mask(dataloaders["Rapgap"]["jet_qt"], Rapgap_pt_mask)
            / ak.mask(dataloaders["Rapgap"]["jet_pt"], Rapgap_pt_mask)
            < 0.3
        )
        Rapgap_number_of_valid_jets = ak.count(Rapgap_valid_jet_mask)
        Rapgap_mask = (dataloaders["Rapgap"]["EEC_energyweight"] > 0) & Rapgap_qt_mask
        Rapgap_RL_masked = ak.flatten(
            ak.drop_none(ak.mask(dataloaders["Rapgap"]["R_L"], Rapgap_mask)), axis=2
        )
        Rapgap_RL_masked = ak.flatten(Rapgap_RL_masked)
        print(len(Rapgap_RL_masked))
        Rapgap_energyweight_masked = ak.flatten(
            ak.drop_none(
                ak.mask(dataloaders["Rapgap"]["EEC_energyweight"], Rapgap_mask)
            ),
            axis=2,
        )
        num_Rapgap_entries_per_event = ak.count(Rapgap_energyweight_masked, axis=1)
        Rapgap_energyweight_masked = ak.flatten(Rapgap_energyweight_masked)

        weights[data_name] = (
            2
            * np.repeat(
                dataloaders["Rapgap"]["mc_weights"]
                * dataloaders["Rapgap"][weight_name],
                num_Rapgap_entries_per_event,
                axis=0,
            )
            * (Rapgap_energyweight_masked)
            / Rapgap_number_of_valid_jets
        )
        weights["Rapgap"] = (
            2
            * np.repeat(
        ...
        feed_dict[data_name] = Rapgap_RL_masked
        feed_dict["Rapgap"] = Rapgap_RL_masked

        # Configuring Djangoh
        if pt_high == None:
            Djangoh_pt_mask = dataloaders["Djangoh"]["jet_pt"] > pt_low
        else:
            Djangoh_pt_mask = (dataloaders["Djangoh"]["jet_pt"] > pt_low) & (
                dataloaders["Djangoh"]["jet_pt"] < pt_high
            )

        Djangoh_qt_mask_nonrepeated = (
            dataloaders["Djangoh"]["jet_qt"][Djangoh_pt_mask]
            / dataloaders["Djangoh"]["jet_pt"][Djangoh_pt_mask]
            < 0.3
        )
        Djangoh_mean_pt = np.mean(
            dataloaders["Djangoh"]["jet_pt"][Djangoh_pt_mask][
                Djangoh_qt_mask_nonrepeated
            ]
        )

        Djangoh_qt_mask = np.repeat(
            (
                ak.mask(dataloaders["Djangoh"]["jet_qt"], Djangoh_pt_mask)
                / ak.mask(dataloaders["Djangoh"]["jet_pt"], Djangoh_pt_mask)
                < 0.3
            )[:, :, np.newaxis],
            dataloaders["Djangoh"]["R_L"].shape[2],
            axis=2,
        )
        Djangoh_valid_jet_mask = (
            ak.mask(dataloaders["Djangoh"]["jet_qt"], Djangoh_pt_mask)
            / ak.mask(dataloaders["Djangoh"]["jet_pt"], Djangoh_pt_mask)
            < 0.3
        )
        Djangoh_number_of_valid_jets = ak.count(Djangoh_valid_jet_mask)
        Djangoh_mask = (
            dataloaders["Djangoh"]["EEC_energyweight"] > 0
        ) & Djangoh_qt_mask
        Djangoh_RL_masked = ak.flatten(
            ak.drop_none(ak.mask(dataloaders["Djangoh"]["R_L"], Djangoh_mask)), axis=2
        )
        Djangoh_RL_masked = ak.flatten(Djangoh_RL_masked)
        Djangoh_energyweight_masked = ak.flatten(
            ak.drop_none(
                ak.mask(dataloaders["Djangoh"]["EEC_energyweight"], Djangoh_mask)
            ),
            axis=2,
        )
        num_Djangoh_entries_per_event = ak.count(Djangoh_energyweight_masked, axis=1)
        Djangoh_energyweight_masked = ak.flatten(Djangoh_energyweight_masked)
        weights["Djangoh"] = (
            2
            * np.repeat(
                dataloaders["Djangoh"]["mc_weights"],
                num_Djangoh_entries_per_event,
                axis=0,
            )
            * (Djangoh_energyweight_masked)
            / Djangoh_number_of_valid_jets
        )
        feed_dict["Djangoh"] = Djangoh_RL_masked

        ylabel = r"$\Sigma_{EEC}(R_L)=\Sigma_{jet}\Sigma_{i,j}\frac{p_{T,i}p_{T,j}}{p_{T,jet}^2}/(N_{jets}\cdot \Delta R_L)$"
        ...
        if pt_high is None:
            ...
            phasespace_text = (
                "\n"
                + rf"${pt_low}~GeV < p_{{T}}^{{jet}} < {pt_high}~GeV$"
                + "\n"
                + "$q_{{T}}/p_{{T}}^{{jet}} < 0.3$"
            )
        # Generate histogram plot
        fig, ax, counts_dict = utils.HistRoutine_nodensity(
            feed_dict,
            xlabel=r"$R_{L}=\sqrt{(\Delta \phi)^2 + (\Delta \eta^2)} $",
            ylabel=ylabel,
            weights=weights,
            logy=False,
            logx=True,
            ...
            reference_name="data" if flags.reco else data_name,
            label_loc="upper left",
            uncertainty=np.zeros(len(binning) - 1),
            stat_uncertainty=np.zeros(len(binning) - 1),
            extra_phasespace_text=phasespace_text,
        )

        # Set plot limits and save
        ax.set_ylim(y_limits[i])
        if pt_high is None:
            fig.savefig(f"../plots/{version}_EEC_RL_jetpT_10GeV_all.pdf")
        else:
            fig.savefig(f"../plots/{version}_EEC_RL_jetpT_{pt_low}GeV_{pt_high}GeV.pdf")

        bin_centers = (binning[:-1] + binning[1:]) / 2
        feed_dict["Djangoh"] = bin_centers * Djangoh_mean_pt
        feed_dict["Rapgap"] = bin_centers * Rapgap_mean_pt
        feed_dict[data_name] = bin_centers * Rapgap_mean_pt

        weights["Djangoh"] = (
            counts_dict["Djangoh"] * np.log(Djangoh_mean_pt) / Djangoh_mean_pt
        )
        weights["Rapgap"] = (
            counts_dict["Rapgap"] * np.log(Rapgap_mean_pt) / Rapgap_mean_pt
        )
        weights[data_name] = (
            counts_dict[data_name] * np.log(Rapgap_mean_pt) / Rapgap_mean_pt
        )

        ylabel = r"$ln <p_{T}^{jet}>/<p_{T}^{jet}>\Sigma_{EEC}(R_L)~((GeV/c)^{-1})$"
        fig, ax, _ = utils.HistRoutine_nodensity(
            feed_dict,
            xlabel=r"$ <p_{T}^{jet}> R_{L} (GeV/c)$",
            ylabel=ylabel,
            weights=weights,
            logy=False,
            logx=True,
            binning=binning * Rapgap_mean_pt,
            reference_name="data" if flags.reco else data_name,
            label_loc="upper left",
            uncertainty=np.zeros(len(binning) - 1),
            stat_uncertainty=np.zeros(len(binning) - 1),
        ...
        ax.set_xlim(0.2, 30)
        if pt_high is None:
            fig.savefig(f"../plots/{version}_EEC_RL_jetpT_10GeV_all_shifted.pdf")
        else:
            fig.savefig(
                f"../plots/{version}_EEC_RL_jetpT_{pt_low}GeV_{pt_high}GeV_shifted.pdf"
            )
...
def plot_observable(flags, var, dataloaders, version):
    ...
    if flags.sys:
        nominal_weights = (
            dataloaders["Rapgap"]["weights"]
            if not flags.reco
            else np.ones_like(dataloaders["Rapgap"]["weights"])
        )

        nominal, _ = compute_histogram(
            "Rapgap", nominal_weights * dataloaders["Rapgap"]["mc_weights"]
        )
        nominal_closure, _ = compute_histogram(
            "Djangoh", dataloaders["Djangoh"]["mc_weights"]
        )
        ...
        for sys in dataloaders:
            ...
            sys_weights = (
                dataloaders[sys]["closure_weights"]
                if sys == "Rapgap"
                else dataloaders[sys]["weights"]
            )
            if flags.reco:
                sys_weights = np.ones_like(sys_weights)
            sys_hist, _ = compute_histogram(
                sys, dataloaders[sys]["mc_weights"] * sys_weights
            )
        ...
        if flags.reco:
            ...
            if flags.bootstrap:
                ...
                for boot in range(1, flags.nboot):
                    ...
                    sys_hist, _ = compute_histogram(
                        "bootstrap",
                        dataloaders["bootstrap"]["mc_weights"] * sys_weights,
                    )
                    stat_unc.append(sys_hist)
                stat_unc = np.ma.divide(
                    np.std(stat_unc, 0), np.mean(stat_unc, 0)
                ).filled(0)
                data_stat_unc = stat_unc
                total_unc += stat_unc**2
                print(f"{sys}: max uncertainty = {np.max(stat_unc)}")
    ...
    if len(dataloaders["Rapgap"][var].shape) > 1:
        if "zh" in var or "jt" in var:
            if "jt" in var:
                ...
                qt_mask = np.repeat(
                    (
                        ak.mask(dataloaders["Rapgap"]["jet_qt"], pt_mask)
                        / ak.mask(dataloaders["Rapgap"]["jet_pt"], pt_mask)
                        < 0.3
                    )[:, :, np.newaxis],
                    132,
                    axis=2,
                ...
                Rapgap_mask = (
                    (dataloaders["Rapgap"][var] > 0)
                    & (dataloaders["Rapgap"]["zh"] > 0.1)
                    & (dataloaders["Rapgap"]["zh"] < 0.5)
                    & qt_mask
                )
            else:
                pt_mask = dataloaders["Rapgap"]["jet_pt"] > 0
            ...
            Rapgap_data = ak.flatten(
                ak.drop_none(ak.mask(dataloaders["Rapgap"][var], Rapgap_mask)), axis=2
            )
        else:
        ...
        feed_dict[data_name] = dataloaders["Rapgap"][var][
            dataloaders["Rapgap"]["jet_pt"] > 0
        ]
    ...
    if len(dataloaders["Djangoh"][var].shape) > 1:
        if "zh" in var or "jt" in var:
            if "jt" in var:
                ...
                qt_mask = np.repeat(
                    (
                        ak.mask(dataloaders["Djangoh"]["jet_qt"], pt_mask)
                        / ak.mask(dataloaders["Djangoh"]["jet_pt"], pt_mask)
                        < 0.3
                    )[:, :, np.newaxis],
                    132,
                    axis=2,
                ...
                Djangoh_mask = (
                    (dataloaders["Djangoh"][var] > 0)
                    & (dataloaders["Djangoh"]["zh"] > 0.1)
                    & (dataloaders["Djangoh"]["zh"] < 0.5)
                    & qt_mask
                )
            else:
                pt_mask = dataloaders["Djangoh"]["jet_pt"] > 0
            ...
            Djangoh_data = ak.flatten(
                ak.drop_none(ak.mask(dataloaders["Djangoh"][var], Djangoh_mask)), axis=2
            )
        else:
            Djangoh_mask = dataloaders["Djangoh"]["jet_pt"] > 0
            Djangoh_data = ak.drop_none(
                ak.mask(dataloaders["Djangoh"][var], Djangoh_mask)
            )

        num_Djangoh_jets_per_event = ak.count(Djangoh_data, axis=1)
        Djangoh_data = ak.flatten(Djangoh_data)
        weights["Djangoh"] = np.repeat(
            dataloaders["Djangoh"]["mc_weights"], num_Djangoh_jets_per_event, axis=0
        )
        feed_dict["Djangoh"] = Djangoh_data
    else:
        weights["Djangoh"] = dataloaders["Djangoh"]["mc_weights"][
    ...
    if flags.reco:
        if len(dataloaders["data"][var].shape) > 1:
            ...
            weights["data"] = np.ones_like(
                dataloaders["data"][var][dataloaders["data"]["jet_pt"] > 0]
            )
            feed_dict["data"] = dataloaders["data"][var][
                dataloaders["data"]["jet_pt"] > 0
            ]
...
def plot_part_observable(flags, var, dataloaders, version):
    ...
    if flags.eec:
        ...
        Rapgap_E_wgt = ak.drop_none(
            ak.mask(dataloaders["Rapgap"]["E_wgt"], Rapgap_mask)
        )
        Rapgap_E_wgt = ak.flatten(Rapgap_E_wgt)
        weights["Rapgap"] = np.repeat(
            dataloaders["Rapgap"]["mc_weights"], num_Rapgap_parts_per_event, axis=0
        )
        weights["Rapgap"] = np.multiply(weights["Rapgap"], Rapgap_E_wgt)
        weights[data_name] = np.repeat(
            dataloaders["Rapgap"]["mc_weights"] * dataloaders["Rapgap"][weight_name],
            num_Rapgap_parts_per_event,
            axis=0,
        )
    ...
    if flags.eec:
        ...
        Djangoh_E_wgt = ak.drop_none(
            ak.mask(dataloaders["Djangoh"]["E_wgt"], Djangoh_mask)
        )
...
def gather_data(dataloaders):
    for dataloader in dataloaders:
        ...
        dataloaders[dataloader].zh = hvd.allgather(
            tf.constant(dataloaders[dataloader].zh)
        ).numpy()
        dataloaders[dataloader].jt = hvd.allgather(
            tf.constant(dataloaders[dataloader].jt)
        ).numpy()
        dataloaders[dataloader].jt_photon = hvd.allgather(
            tf.constant(dataloaders[dataloader].jt_photon)
        ).numpy()
        dataloaders[dataloader].EEC_energyweight = hvd.allgather(
            tf.constant(dataloaders[dataloader].EEC_energyweight)
        ).numpy()
        dataloaders[dataloader].R_L = hvd.allgather(
            tf.constant(dataloaders[dataloader].R_L)
        ).numpy()
