...
class Dataset():
    def __init__(self,
        ...
        self.mean_part = [0.0 , 0.0, -0.761949, -3.663438,
                          -2.8690917,0.03239748, 3.9436243]
        self.std_part =  [1.0, 1.0, 1.0133458, 1.03931,
                          1.0040112, 0.98908925, 1.2256976] 
        ...
        self.prepare_dataset(file_names,pass_fiducial)
    ...
    def prepare_dataset(self,file_names,pass_fiducial):
        ...
        for ifile, f in enumerate(file_names):
            ...
            if self.nmax is None:
                self.nmax = h5.File(os.path.join(self.base_path,f),'r')['gen_event_features'].shape[0]

            # Just say all events pass reconstruction
            self.num_pass_reco += np.sum(np.ones(self.nmax))
            ...
            if self.is_mc:
                ...
                with h5.File(os.path.join(self.base_path, f), 'r') as file:
                    pass_gen_entries = file['gen_event_features'][:self.nmax, -1]
                    self.num_pass_gen += np.sum(pass_gen_entries == 1)

                if pass_fiducial:
                    mask_fid = self.pass_gen[-1]
                else:
                    mask_fid = np.ones_like(self.pass_gen[-1])                    
                gen_p = gen_p[mask_fid]
                gen_e = gen_e[mask_fid]
                self.pass_gen[-1] = self.pass_gen[-1][mask_fid]
            ...
            # Say all events have weight 1 and all pass reconstruction

            weights = np.ones_like(mask_fid)
            self.weight.append(weights.astype(np.float32))

            pass_reco = np.full_like(mask_fid, True)
            self.pass_reco.append(pass_reco)
            print(self.weight[-1].shape, mask_fid.shape, self.pass_reco[-1].shape)
            self.weight[-1] = self.weight[-1][mask_fid]
            self.pass_reco[-1] = self.pass_reco[-1][mask_fid]
