...
if __name__ == "__main__":
    ...
    dataloader_data = Dataset(
        file_data, base_path, is_mc=False, norm=dataloader_mc.nmax
    )
    ...
    print(
        "Pass gen fid MC: {}".format(
            1.0 * np.sum(dataloader_mc.pass_gen) / pass_reco_mc.shape[0]
        )
    )
    print(
        "Pass fid but not pass reco {}".format(
            1.0
            * np.sum(dataloader_mc.pass_gen[pass_reco_mc == 0])
            / np.sum(dataloader_mc.pass_gen)
        )
    )
    print(
        "Pass fid  pass reco {}".format(
            1.0
            * np.sum(dataloader_mc.pass_gen[pass_reco_mc])
            / np.sum(dataloader_mc.pass_gen)
        )
    )
    print(
        "Not pass fid but pass reco {}".format(
            1.0
            * np.sum(dataloader_mc.pass_gen[pass_reco_mc] == 0)
            / np.sum(dataloader_mc.pass_gen == 0)
        )
    )

    print(
        "Pass reco to pass fiducial ratio: {}".format(
            1.0 * np.sum(pass_reco_mc) / np.sum(dataloader_mc.pass_gen)
        )
    )
    ...
    particles_data, events_data = dataloader_data.revert_standardize(
        particles_data, events_data, mask_data
    )

    print("Using {} particles for data".format(particles_data.shape[1]))
    pass_reco_data = dataloader_data.pass_reco
    print(
        "Sum of weights data: {}".format(np.sum(dataloader_data.weight[pass_reco_data]))
    )
    print("Total data: {}".format(pass_reco_data.shape[0]))
    print(
        "Pass reco data: {}".format(
            1.0 * np.sum(pass_reco_data) / pass_reco_data.shape[0]
        )
    )
