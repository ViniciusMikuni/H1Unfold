...
class Multifold():
    def __init__(self,
        ...
        self.weights_folder = '../weights_fiducial_1million'
        # if self.nstrap>0:
    ...
    def RunStep1(self,i):
        ...
        if self.verbose:
            ...
            print("Full number of events {}".format(np.concatenate((self.labels_mc[self.mc.pass_gen] , self.labels_data[self.data.pass_gen])).shape[0]))

        
        print("Weights:",np.sum(self.weights_push[self.mc.pass_gen]*self.mc.weight[self.mc.pass_gen]), np.sum(self.data.weight[self.data.pass_gen]))
        self.RunModel(
            np.concatenate((self.labels_mc[self.mc.pass_gen], self.labels_data[self.data.pass_gen])),
            np.concatenate((self.weights_push[self.mc.pass_gen]*self.mc.weight[self.mc.pass_gen],self.data.weight[self.data.pass_gen])),
        ...
        new_weights[self.mc.pass_gen] = self.reweight(self.mc.gen,self.model1_ema,batch_size=1000)[self.mc.pass_gen]
        ...
        if not cached:
            ...
            if stepn ==1:
                ...
                self.tf_data1 = tf.data.Dataset.from_tensor_slices(
                    {'inputs_particle_1':np.concatenate((self.mc.gen[0][self.mc.pass_gen], self.data.gen[0][self.data.pass_gen]))[self.idx_1],
                     'inputs_event_1':np.concatenate((self.mc.gen[1][self.mc.pass_gen], self.data.gen[1][self.data.pass_gen]))[self.idx_1],
                     'inputs_mask_1':np.concatenate((self.mc.gen[2][self.mc.pass_gen], self.data.gen[2][self.data.pass_gen]))[self.idx_1],
                     })
    ...
    def CompileModel(self,lr,fixed=False):
        ...
        if self.num_steps_reco ==None:
            self.num_steps_reco = int(0.7*(self.mc.num_pass_gen + self.data.num_pass_gen))//hvd.size()//self.BATCH_SIZE
    ...
    def PrepareInputs(self):
        self.labels_mc = np.zeros(len(self.mc.pass_gen),dtype=np.float32)
        self.labels_data = np.ones(len(self.data.pass_gen),dtype=np.float32)
