...
def get_version(flags, jobID, opt):
    '''OmniFold_H1_April_Pretrain20M_ParallelJobs_closure_job57_pretrained_iter3_step1'''
...
def load_model(flags, opt, version, dataloaders):
    ...
    '''{}/OmniFold_{version}_iter4_step2.pkl''' #large jobID, no ensemble Avg yet
...
def main():
    ...
    # for jobID in range(47,flags.n_jobs):
    for jobID in range(44,flags.n_jobs):
        if jobID == 48: continue
        if jobID == 31: continue
        ...
        if flags.load:
            ...
            weights = load_model(flags, opt, version, dataloaders)  #FIXME: JobID does nothing, r
            np.save(f"../weights/{version}_weights.npy",weights) #already re-weighted, L:109
        if hvd.rank()==0:
